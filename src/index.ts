...
export = (app: Probot) => {
  ...
  app.on("issue_comment.created", async (context) => {
    if (context.payload.issue.html_url.includes("pull") && context.payload.comment.body.includes('レビューお願いします') && context.payload.issue.user.login === context.payload.comment.user.login) {

      if (context.payload.issue.labels.some(label => label.name.includes('WIP'))) {
  ...
  app.on("pull_request_review_comment.created", async (context) => {
    if (context.payload.comment.html_url.includes("pull") && context.payload.comment.body.includes('レビューお願いします') && context.payload.comment.user.login === context.payload.comment.user.login) {

      if (context.payload.pull_request.labels.some(label => label.name.includes('WIP'))) {
        const removeLabelParams = context.issue({
          name: 'WIP',
        });
        await context.octokit.issues.removeLabel(removeLabelParams);
  ...
  app.on("pull_request_review.submitted", async (context) => {
    if (context.payload.review.html_url.includes("pull") && context.payload.review.state === 'commented' && context.payload.review.user.login === context.payload.pull_request.user.login) {
  ...
  app.on("pull_request_review.submitted", async (context) => {
    if (context.payload.review.state === 'changes_requested') {
      if (context.payload.pull_request.labels.some(label => label.name.includes('Needs review'))) {
        const removeLabelParams = context.issue({
          name: 'Needs review',
        });
        await context.octokit.issues.removeLabel(removeLabelParams);
  ...
  app.on("pull_request.reopened", async (context) => {
    if (context.payload.pull_request.labels.some(label => label.name.includes('Close pull request'))) {
      const removeLabelParams = context.issue({
